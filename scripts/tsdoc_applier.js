    ...
    const processFile = async (tsFile) => {
      ...
      if (changed) {
        ...
        if (updatedLines >= originalLines) {
          ...
          console.log(`${dryRun ? '[DRY RUN] Would update' : 'Updated'} ${tsFile.path} with ${nodes.length} comments`);
          return result;
        } else {
          console.error(`Error: Updated file has fewer lines than original. Skipping: ${tsFile.path}`);
          return null;
        }
      } else if (verbose) {
...
async function main() {
  try {
    ...
    console.log(`Found ${commentFiles.length} comment files with a total of ${commentFiles.reduce((sum, file) => sum + file.totalComments, 0)} comments`);
    console.log(`- Auto-generated comments: ${commentFiles.reduce((sum, file) => sum + file.autoGenerated, 0)}`);
    console.log(`- Manual comments: ${commentFiles.reduce((sum, file) => sum + file.manual, 0)}\n`);
    
    // Ask if user wants to apply comments
    const applyResponse = await prompt('Do you want to apply these comments to the source files? (yes/no/dry): ');
    ...
    const concurrencyResponse = await prompt('How many files to process concurrently? (default: 10): ');
    ...
    for (const packagePath of packages) {
      ...
      updatedFilesStats.totalCommentsAdded += updatedFiles.reduce((sum, file) => sum + file.addedComments, 0);
      
      // Display per-file stats
      for (const file of updatedFiles) {
        console.log(`  - ${path.basename(file.path)}: ${dryRun ? 'Would add' : 'Added'} ${file.addedComments} comments (${file.originalLines} → ${file.updatedLines} lines)`);
      }
    }
    
    // Display final stats
    console.log('\n=== Final Results ===');
    console.log(`Files ${dryRun ? 'that would be updated' : 'updated'}: ${updatedFilesStats.updatedFiles}`);
    console.log(`Total comments ${dryRun ? 'that would be added' : 'added'}: ${updatedFilesStats.totalCommentsAdded}`);
