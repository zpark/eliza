 ...
 */
async function sendDiscordMessage(runtime: IAgentRuntime, discordHandle: string, message: string): Promise<boolean> {
  try {
...
export const checkInTeamMember: Action = {
  ...
  description: "Records a daily check-in for a team member or sends reminders to team members who haven't checked in",
  ...
  handler: async (
    ...
    try {
      ...
      const typedTeamMemberMemories = teamMemberMemories as Array<Memory & { content: TeamMemberContent }>;
      ...
      if (isReminder) {
        ...
        const dailyUpdateMemories = await runtime.getMemoryManager("messages").getMemories({
          roomId: dailyUpdatesRoomId
        });
        
        const typedDailyUpdateMemories = dailyUpdateMemories as Array<Memory & { content: DailyUpdateContent }>;
        ...
        const projectTeamMembers = typedTeamMemberMemories.filter(memory => {
          ...
          return member.projects && member.projects.includes(projectId as UUID);
        });
        ...
        for (const memberMemory of projectTeamMembers) {
          ...
          const hasCheckedInToday = typedDailyUpdateMemories.some(memory => 
      ...
      logger.info(`Recording check-in for team member ID: ${teamMemberId}, project ID: ${projectId}`);
      
      // Find the team member with the given ID
      const teamMemberMemory = typedTeamMemberMemories
      ...
      const existingUpdateMemory = (dailyUpdateMemories as Array<Memory & { content: DailyUpdateContent }>)
        .find(memory => 
      ...
      if (existingUpdateMemory) {
        logger.warn(`Team member ${teamMemberId} already checked in for project ${projectId} today`);
  ...
  examples: [
    [
      ...
      {
        name: "jimmy",
        content: { text: "I'll record your check-in for project Alpha. Please provide a summary of your work today.", actions: ["checkInTeamMember"] },
      },
    ],
    [
      {
      ...
      {
        name: "jimmy",
        content: { text: "I'll send reminders to all team members who haven't checked in yet for Project Beta.", actions: ["checkInTeamMember"] },
      },
    ],
  ],
};
