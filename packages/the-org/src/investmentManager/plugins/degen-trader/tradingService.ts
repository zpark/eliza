...
export class DegenTradingService extends Service {
	...
	private async getTwitterSignals(): Promise<TokenSignal[]> {
		try {
			...
			const twitterSignals =
				(await this.runtime
					.getDatabaseAdapter()
					.getCache<any[]>("twitter_parsed_signals")) || [];
	...
	private async getCMCSignals(): Promise<TokenSignal[]> {
		try {
			...
			const cmcTokens =
				(await this.runtime
					.getDatabaseAdapter()
					.getCache<any[]>("cmc_trending_tokens")) || [];
	...
	async handlePriceSignal(signal: PriceSignalMessage): Promise<void> {
		...
		await this.runtime
			.getDatabaseAdapter()
			.setCache<any>(`price:${signal.tokenAddress}`, {
				initialPrice: signal.initialPrice,
				currentPrice: signal.currentPrice,
				priceChange: signal.priceChange,
	...
	async handleBuySignal(signal: BuySignalMessage): Promise<{
		...
		try {
			...
			const tradeResult = await executeTrade(this.runtime, {
				...
				slippage:
					Number(
						this.calculateDynamicSlippage(signal.tokenAddress, tradeAmount),
	...
	private async executeSellTask(options: any) {
		try {
			...
			if (
				...
				Number(signal.amount) > Number(signal.currentBalance)
			) {
		...
		try {
			...
			const analysis = {
				...
				recommendation: {
					...
					score: recommendation.marketcap
						? recommendation.marketcap / 1000000
						: 0, // Normalize market cap as a score
				},
			};
	...
	async startDegenProcess(data: {
		...
		try {
			...
			await this.runtime
				.getDatabaseAdapter()
				.setCache(`token_monitor:${data.tokenAddress}`, {
					id: data.id,
					initialPrice,
		...
		try {
			...
			if (priceChangePercent > 0) {
				const technicalSignals =
					await this.calculateTechnicalSignals(marketData);
	...
	async setTrailingStop(
		...
		try {
			...
			await this.runtime
				.getDatabaseAdapter()
				.setCache(`trailing_stop:${tokenAddress}`, {
					tokenAddress,
	...
	async monitorTrailingStop(options: { tokenAddress: string }): Promise<void> {
		try {
			...
			if (!currentBalance || BigInt(currentBalance.toString()) <= BigInt(0)) {
				...
				await this.runtime
					.getDatabaseAdapter()
					.deleteCache(`trailing_stop:${tokenAddress}`);
				return;
			}
			...
			if (marketData.price > trailingStop.highestPrice) {
				...
				await this.runtime
					.getDatabaseAdapter()
					.setCache(`trailing_stop:${tokenAddress}`, trailingStop);
	...
	private async storePerformanceSnapshot(
		...
		try {
			...
			await this.runtime
				.getDatabaseAdapter()
				.setCache(`performance_snapshot:${snapshot.timestamp}`, snapshot);

			// Update snapshot index
			const snapshotIndex =
				(await this.runtime
					.getDatabaseAdapter()
					.getCache<string[]>("performance_snapshot_index")) || [];
			...
			await this.runtime
				.getDatabaseAdapter()
				.setCache("performance_snapshot_index", snapshotIndex);
		} catch (error) {
	...
	private async updatePositionData(
		...
		try {
			...
			await this.runtime
				.getDatabaseAdapter()
				.setCache(`position:${tokenAddress}`, {
					tokenAddress,
					amount,
					price: marketData.price,
					value,
	...
	private async getTradesForToken(tokenAddress: string): Promise<any[]> {
		try {
			...
			const allTrades =
				(await this.runtime
					.getDatabaseAdapter()
					.getCache<string[]>(allTradesKey)) || [];

			// Filter trades for this token
			const tokenTrades = [];
			for (const tradeKey of allTrades) {
				if (tradeKey.startsWith(tokenAddress)) {
					const trade = await this.runtime
						.getDatabaseAdapter()
						.getCache<any>(`trade_performance:${tradeKey}`);
					if (trade) {
	...
	private async updateTokenStatistics(
		...
		try {
			...
			await this.runtime
				.getDatabaseAdapter()
				.setCache(`token_stats:${tokenAddress}`, stats);
		} catch (error) {
	...
	private async calculateSellAmountWithMarketContext(
		...
		try {
			...
			const sellAmount =
				(balanceNum * BigInt(Math.floor(sellPercentage * 100))) / BigInt(100);
	...
	async addDegenTransaction(data: {
		...
		try {
			...
			await this.runtime
				.getDatabaseAdapter()
				.setCache<any>(`transaction:${data.id}`, transactionData);
	...
	async getLatestTradePerformance(
		...
		try {
			...
			const performanceIndex =
				(await this.runtime
					.getDatabaseAdapter()
					.getCache<any[]>("trade_performance_index")) || [];
			...
			if (filtered.length > 0) {
				...
				const fullRecord = await this.runtime
					.getDatabaseAdapter()
					.getCache<any>(`trade_performance:${latestEntry.id}`);
	...
	private async calculateDrawdown(portfolio: {
		...
		try {
			...
			const hwm =
				(await this.runtime
					.getDatabaseAdapter()
					.getCache<number>("portfolio_hwm")) || portfolio.totalValue;

			// Update high water mark if current value is higher
			if (portfolio.totalValue > hwm) {
				await this.runtime
					.getDatabaseAdapter()
					.setCache("portfolio_hwm", portfolio.totalValue);
				return 0;
			}
		...
		try {
			...
			if (await this.hasSpecialSlippageRequirement(tokenAddress)) {
				...
				const specialAdjustment =
					await this.getSpecialSlippageAdjustment(tokenAddress);
			...
			const finalSlippage = Math.min(
				slippage,
				this.tradingConfig.slippageSettings.maxSlippage,
			);
	 ...
	 */
	private async hasSpecialSlippageRequirement(
		tokenAddress: string,
	): Promise<boolean> {
	 ...
	 */
	private async getSpecialSlippageAdjustment(
		tokenAddress: string,
	): Promise<number> {
		try {
	...
	private async trackSlippageImpact(
		...
		try {
			...
			await this.runtime
				.getDatabaseAdapter()
				.setCache(`slippage_impact:${tokenAddress}:${Date.now()}`, {
					tokenAddress,
					timestamp: new Date().toISOString(),
					expectedAmount,
					actualAmount,
	...
	private async maybeOptimizeSlippageParameters(): Promise<void> {
		try {
			...
			const slippageKeys =
				(await this.runtime
					.getDatabaseAdapter()
					.getCache<string[]>("slippage_impact_keys")) || [];
			...
			if (highVolumeRecords.length >= 5) {
				const highVolumeEfficiency =
					this.calculateAverageSlippageEfficiency(highVolumeRecords);
			...
			if (
				optimizedSettings.liquidityMultiplier !==
					this.tradingConfig.slippageSettings.liquidityMultiplier ||
				optimizedSettings.volumeMultiplier !==
					this.tradingConfig.slippageSettings.volumeMultiplier
			) {
				...
				await this.runtime
					.getDatabaseAdapter()
					.setCache("slippage_settings", optimizedSettings);
			...
			await this.runtime
				.getDatabaseAdapter()
				.setCache("last_slippage_optimization", now);
		} catch (error) {
	...
	private calculateAverageSlippageEfficiency(records: any[]): number {
		...
		const filteredEfficiencies = efficiencies.filter(
			(e) => e >= lowerBound && e <= upperBound,
		);

		// Calculate average of non-outlier values
		return (
			filteredEfficiencies.reduce((sum, val) => sum + val, 0) /
			filteredEfficiencies.length
		);
	}
		...
		try {
			...
			if (!signal.amount || signal.amount === "auto") {
				...
				const calculatedAmount =
					await this.calculateSellAmountWithMarketContext(
	...
	private async executeBuyTask() {
		try {
			...
			const buyTasks = pendingTasks.filter(
				(task) => task.name === "BUY_SIGNAL",
			);
