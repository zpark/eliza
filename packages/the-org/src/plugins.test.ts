...
import type { IDatabaseAdapter, TestCase } from "@elizaos/core";
import {
  AgentRuntime,
  logger,
  stringToUuid,
...
import { afterAll, beforeAll, describe, it, RuntimeConfig } from "vitest";
...
async function initializeRuntime(character: Character): Promise<IAgentRuntime> {
  try {
    ...
    const adapter = drizzleAdapter.createDatabaseAdapter({}, runtime.agentId);
    if (!adapter) {
