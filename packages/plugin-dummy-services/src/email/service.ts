 ...
 */
export class DummyEmailService extends Service implements IEmailService {
  static override readonly serviceType = IEmailService.serviceType;
  
  public readonly capabilityDescription = 'Dummy email service for testing';
  ...
  async getEmails(options?: EmailSearchOptions): Promise<EmailMessage[]> {
    ...
    if (options?.from) {
      filteredEmails = filteredEmails.filter(email => 
        email.from.email.includes(options.from!) || 
        email.from.name?.includes(options.from!)
      );
    }
    ...
    if (options?.since) {
      filteredEmails = filteredEmails.filter(email => 
        email.date && email.date >= options.since!
      );
    }
    
    if (options?.before) {
      filteredEmails = filteredEmails.filter(email => 
        email.date && email.date <= options.before!
      );
    }
  ...
  async createFolder(folderName: string, parentPath?: string): Promise<void> {
    this.runtime.logger.debug(`Creating folder ${folderName}${parentPath ? ` under ${parentPath}` : ''}`);
