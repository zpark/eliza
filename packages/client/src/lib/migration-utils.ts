...
export function createHybridClient() {
  ...
  return {
    ...
    getServers: MIGRATION_FLAGS.USE_NEW_MESSAGING_API
      ? wrapWithErrorHandling(newClient.messaging?.getServers?.bind(newClient.messaging) || (() => { throw new Error('Messaging service not available'); }))
      : legacyClient.getServers,
    getChannels: MIGRATION_FLAGS.USE_NEW_MESSAGING_API
      ? wrapWithErrorHandling(newClient.messaging?.getChannels?.bind(newClient.messaging) || (() => { throw new Error('Messaging service not available'); }))
      : legacyClient.getChannels,
    getChannelMessages: MIGRATION_FLAGS.USE_NEW_MESSAGING_API
      ? wrapWithErrorHandling(newClient.messaging?.getChannelMessages?.bind(newClient.messaging) || (() => { throw new Error('Messaging service not available'); }))
      : legacyClient.getChannelMessages,
    postMessageToChannel: MIGRATION_FLAGS.USE_NEW_MESSAGING_API
      ? wrapWithErrorHandling(newClient.messaging?.sendMessage?.bind(newClient.messaging) || (() => { throw new Error('Messaging service not available'); }))
      : legacyClient.postMessageToChannel,
    
    // Memory services
    getAgentMemories: MIGRATION_FLAGS.USE_NEW_MEMORY_API
      ? wrapWithErrorHandling(newClient.memory?.getAgentMemories?.bind(newClient.memory) || (() => { throw new Error('Memory service not available'); }))
      : legacyClient.getAgentMemories,
    
    // Media services
    uploadAgentMedia: MIGRATION_FLAGS.USE_NEW_MEDIA_API
      ? wrapWithErrorHandling(newClient.media?.uploadAgentMedia?.bind(newClient.media) || (() => { throw new Error('Media service not available'); }))
      : legacyClient.uploadAgentMedia,
    uploadChannelMedia: MIGRATION_FLAGS.USE_NEW_MEDIA_API
      ? wrapWithErrorHandling(newClient.media?.uploadChannelMedia?.bind(newClient.media) || (() => { throw new Error('Media service not available'); }))
      : legacyClient.uploadChannelMedia,
    
    // Audio services
    ttsStream: MIGRATION_FLAGS.USE_NEW_AUDIO_API
      ? wrapWithErrorHandling(newClient.audio?.generateSpeech?.bind(newClient.audio) || (() => { throw new Error('Audio service not available'); }))
      : legacyClient.ttsStream,
    transcribeAudio: MIGRATION_FLAGS.USE_NEW_AUDIO_API
      ? wrapWithErrorHandling(newClient.audio?.transcribeAudio?.bind(newClient.audio) || (() => { throw new Error('Audio service not available'); }))
      : legacyClient.transcribeAudio,
    
    // System services
    ping: MIGRATION_FLAGS.USE_NEW_SYSTEM_API
      ? wrapWithErrorHandling(newClient.system?.ping?.bind(newClient.system) || (() => { throw new Error('System service not available'); }))
      : legacyClient.ping,
