...
export default function AgentRoute() {
  ...
  const agentFromHook: Agent | undefined = agentDataResponse?.data
    ? ({
      ...
      enabled: typeof agentDataResponse.data.enabled === 'boolean' ? agentDataResponse.data.enabled : true,
      createdAt: typeof agentDataResponse.data.createdAt === 'number' ? agentDataResponse.data.createdAt : Date.now(),
      updatedAt: typeof agentDataResponse.data.updatedAt === 'number' ? agentDataResponse.data.updatedAt : Date.now(),
    } as Agent)
    : undefined;
  ...
  if (isLoadingAgent || !agentFromHook) return <div className="p-4 flex items-center justify-center h-full"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>;
  ...
  if (!isActive) {
    clientLogger.info('[AgentRoute] Agent is not active, rendering inactive state UI', { agentName: agentFromHook?.name });
    return (
  ...
  clientLogger.info('[AgentRoute] Agent is active, rendering UnifiedChatView for DM', { agentName: agentFromHook?.name });
