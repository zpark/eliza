...
export function ServerManagement({ open, onOpenChange }: ServerManagementProps) {
    ...
    const handleRemoveAgentFromServer = async (serverId: UUID, agentId: UUID) => {
        ...
        try {
            ...
            setServerAgents(prev => {
                ...
                newMap.set(serverId, agents.filter(id => id !== agentId));
                return newMap;
            });
    ...
    const getAvailableAgents = () => {
        ...
        return agentsData.data.agents.filter(agent =>
            agent.id && !currentAgents.includes(agent.id as UUID)
        );
    };
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-4xl max-h-[80vh] overflow-hidden">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Manage server-agent associations. Add or remove agents from servers to control which agents can process messages.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <Select value={selectedServerId || undefined} onValueChange={(value) => setSelectedServerId(value as UUID)}>
                            <SelectTrigger>
                                    ...
                                    <div className="space-y-2">
                                        {(serverAgents.get(selectedServerId) || []).length === 0 ? (
                                            ...
                                            (serverAgents.get(selectedServerId) || []).map((agentId) => (
                                                <div key={agentId} className="flex items-center justify-between p-2 rounded-lg border">
                    ...
                    {selectedServerId && (
                        <div className="space-y-2">
                            ...
                            <div className="flex gap-2">
                                <Select value={selectedAgentId || undefined} onValueChange={(value) => setSelectedAgentId(value as UUID)}>
