    ...
    async (agentIdForNewChannel: UUID | undefined) => {
      ...
      if (latestChannel) {
        try {
          ...
          const latestMessages = await elizaClient.messaging.getChannelMessages(latestChannel.id, { limit: 30 });
          ...
          if (hasAutoName && isEmpty) {
            clientLogger.info('[Chat] Latest DM channel is empty with auto-generated name or has no messages, reusing instead of creating.');
            updateChatState({ currentDmChannelId: latestChannel.id });
            return;
          } else {
            clientLogger.info('[Chat] Latest DM channel has messages or customized name, proceeding to create a new one.');
          }
        } catch (error) {
