  ...
  const handleNewDmChannel = useCallback(
    async (agentIdForNewChannel: UUID | undefined) => {
      ...
      if (
        ...
        if (isAutoGeneratedChatName(latestChannel.name)) {
          clientLogger.info('[Chat] Latest DM channel is empty with auto-generated name, reusing instead of creating.');
          updateChatState({ currentDmChannelId: latestChannel.id });
          return;
        } else {
          clientLogger.info('[Chat] Latest DM channel is empty but name has been customized; proceeding to create a new one.');
        }
      }
