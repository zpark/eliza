...
describe('LocalAI Image Description', () => {
  ...
  test('should describe image from URL successfully', async () => {
    ...
    try {
      const result = await mockRuntime.useModel(ModelClass.IMAGE_DESCRIPTION, imageUrl);
  ...
  test('should handle non-string input', async () => {
    ...
    try {
      await mockRuntime.useModel(ModelClass.IMAGE_DESCRIPTION, invalidInput as unknown);
      throw new Error("Should have failed but didn't");
    } catch (error) {
