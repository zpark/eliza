...
vi.mock('node:child_process', () => {
  ...
  return {
    exec: vi.fn().mockImplementation(
      (
        command: string,
        options: ExecOptions | undefined | null | ((error: ExecException | null, stdout: string, stderr: string) => void),
...
describe('LocalAI Audio Transcription', () => {
  ...
  test('should transcribe audio buffer successfully', async () => {
    ...
    const result = await mockRuntime.useModel(ModelClass.TRANSCRIPTION, audioBuffer);
  ...
  test('should handle empty audio buffer', async () => {
    ...
    try {
      await mockRuntime.useModel(ModelClass.TRANSCRIPTION, emptyBuffer);
  ...
  test('should handle invalid audio format', async () => {
    ...
    try {
      await mockRuntime.useModel(ModelClass.TRANSCRIPTION, invalidBuffer);
      throw new Error("Should have failed but didn't");
    } catch (error) {
  ...
  test('should handle audio conversion failure', async () => {
    ...
    try {
      await mockRuntime.useModel(ModelClass.TRANSCRIPTION, audioBuffer);
      throw new Error("Should have failed but didn't");
    } catch (error) {
