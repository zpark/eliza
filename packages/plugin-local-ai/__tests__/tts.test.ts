...
import type { LlamaModel, LlamaContext, LlamaContextSequence } from 'node-llama-cpp';
...
describe('LocalAI Text-to-Speech', () => {
  ...
  test('should generate speech from text successfully', async () => {
    ...
    try {
      const result = await mockRuntime.useModel(ModelClass.TEXT_TO_SPEECH, testText);
  ...
  test('should handle non-string input', async () => {
    ...
    try {
      await mockRuntime.useModel(ModelClass.TEXT_TO_SPEECH, invalidInput as unknown as string);
      throw new Error("Should have failed but didn't");
    } catch (error) {
