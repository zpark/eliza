...
export class TokenizerManager {
  ...
  async loadTokenizer(modelConfig: ModelSpec): Promise<PreTrainedTokenizer> {
    try {
      ...
      logger.info("Initializing new tokenizer from HuggingFace with models directory:", this.modelsDir);
      ...
      try {
        ...
        this.tokenizers.set(tokenizerKey, tokenizer);
        logger.success("Tokenizer loaded successfully on retry:", { key: tokenizerKey });
        return tokenizer;
      }
    } catch (error) {
