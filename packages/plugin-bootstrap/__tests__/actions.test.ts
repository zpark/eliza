...
// Additional mock setup for actions testing
function setupActionTest() {
  ...
  // Spy on commonly used methods
  vi.spyOn(logger, 'error').mockImplementation(() => {});
  vi.spyOn(logger, 'warn').mockImplementation(() => {});
  vi.spyOn(logger, 'debug').mockImplementation(() => {});

  // Declare callbackFn in this scope so createMemory can use it
  const callbackFn = vi.fn();

  // Ensure mockMessage.content is never undefined
  if (!mockMessage.content) {
    mockMessage.content = { text: '' };
  }

  return { mockRuntime, mockMessage, mockState, callbackFn };
}
