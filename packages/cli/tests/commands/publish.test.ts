...
describe("ElizaOS Publish Commands", () => {
  ...
  beforeEach(async () => {
    ...
    await writeFile(join(elizaosDataDir, "credentials.json"), JSON.stringify({
      github: {
    ...
    await writeFile(join(elizaosDataDir, "registry.json"), JSON.stringify({
    ...
    await writeFile(join(mockBinDir, "npm"), `#!/bin/bash
...
case "$1" in
  ...
  *)
    ...
    await writeFile(join(mockBinDir, "git"), `#!/bin/bash
...
case "$1" in
  ...
  *)
    ...
    await writeFile(join(mockBinDir, "gh"), `#!/bin/bash
case "$1" in
  "auth")
  ...
  *)
  ...
  const createTestPlugin = async (name: string) => {
    ...
    await writeFile("src/index.ts", `export default {
  name: "test-plugin",
