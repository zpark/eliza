...
async function generateCoverageReport(): Promise<CoverageReport> {
  ...
  for (const sourceFile of sourceFiles) {
    ...
    const hasTest = Array.from(testedFiles).some(tested => 
      tested.includes(relativePath.replace('.ts', '')) ||
      sourceFile.includes(tested)
    );
...
async function main() {
  ...
  if (report.coverage === 100) {
    ...
    console.log(`   ⚠️  ${report.untestedFiles.length} files still need unit tests to reach 100% coverage.`);
