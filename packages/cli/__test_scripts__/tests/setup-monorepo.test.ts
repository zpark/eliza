...
import { setupTestEnvironment, cleanupTestEnvironment, runCliCommand, expectCliCommandToFail, expectHelpOutput, type TestContext } from "./test-utils";
...
describe("ElizaOS Setup Monorepo Commands", () => {
  ...
  test("setup-monorepo --help shows usage", () => {
    const result = runCliCommand(context.elizaosCmd, "setup-monorepo --help");
    expectHelpOutput(result, "setup-monorepo", ["-b", "--branch", "-d", "--dir"]);
  });
  ...
  test("setup-monorepo fails when directory is not empty", async () => {
    ...
    const result = expectCliCommandToFail(context.elizaosCmd, "setup-monorepo --dir not-empty-dir", { timeout: 10000 });
