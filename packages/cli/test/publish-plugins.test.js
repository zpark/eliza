...
const runPublishPluginsScript = async () => {
  const pattern = process.argv.find(arg => arg.startsWith('--pattern='))?.split('=')[1] || '**/plugin-*';
  const all = process.argv.includes('--all');
  const platform = process.argv.find(arg => arg.startsWith('--platform='))?.split('=')[1] || 'universal';
  const registry = process.argv.find(arg => arg.startsWith('--registry='))?.split('=')[1] || 'elizaos/registry';
  ...
  for (const plugin of plugins) {
    try {
      ...
      // Read package.json
      const packageJsonContent = await fsPromises.readFile(path.join(plugin, 'package.json'), 'utf8');
      ...
      if (packageJson.name && packageJson.name.includes('plugin')) {
        ...
        // Publish the plugin
        execSync(`cd ${plugin} && npx elizaos publish --platform ${platform} --registry ${registry}`, 
          { stdio: 'inherit' });
      }
    } catch (error) {
...
describe('Publish Plugins Script', () => {
  ...
  test('should find and process matching plugins', async () => {
    ...
    expect(fsPromises.access).toHaveBeenCalledWith(expect.stringContaining('plugin-test/package.json'));
...
describe('Publish Plugins Script - Non-Plugin Test', () => {
  ...
  test.skip('should skip non-plugin packages', async () => {
    ...
    const publishCalls = vi.mocked(execSync).mock.calls.filter(
      call => call[0].includes('plugin-other') && call[0].includes('publish')
    );
