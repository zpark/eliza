...
export function agentRouter(
	...
	router.post("/:agentId/rooms/:roomId/participants", async (req, res) => {
		...
		try {
			...
			try {
				...
				try {
					...
					if (!entity) {
						...
						logger.info(`[ROOM PARTICIPANT ADD] Participant ${participantId} not found, creating it`);
						...
						logger.info(`[ROOM PARTICIPANT ADD] Creating participant entity: ${JSON.stringify(participantEntity)}`);
					...
					logger.info(`[ROOM PARTICIPANT ADD] Error checking participant, creating it with ID ${participantId}`);
				...
				logger.error(`[ROOM PARTICIPANT ADD] Failed to ensure participant exists: ${entityError.message}`);
				res.status(500).json({
					success: false,
					error: {
						code: "UPDATE_ERROR",
			...
			logger.error(
				`[ROOM PARTICIPANT ADD] Error adding participant to room ${roomId}:`,
				error,
			);
			res.status(500).json({
				success: false,
	...
	router.delete("/:agentId/rooms/:roomId/participants/:participantId", async (req, res) => {
		...
		try {
			...
			const adminEntityId = req.query.adminEntityId as string || req.headers['x-entity-id'] as string;
	...
	router.get("/:agentId/rooms/:roomId/participants", async (req, res) => {
		...
		try {
			...
			logger.error(
				`[ROOM PARTICIPANTS GET] Error getting participants for room ${roomId}:`,
				error,
			);
			res.status(500).json({
				success: false,
