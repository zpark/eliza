...
export class SocketIORouter {
  ...
  private async handleBroadcastMessage(socket: Socket, payload: any) {
    ...
    logger.info(
      `[SocketIO] Known Connections: ${Array.from(this.connections.entries())
    ...
    try {
      ...
      for (const clientSocket of socketsInRoom) {
        ...
        logger.info(
          `[SocketIO] Socket ${clientSocket.id} in room ${roomId} is for entity: ${recipientId || 'unknown'}`
        );
      }
      ...
      for (const clientSocket of socketsInRoom) {
        ...
        if (recipientId === entityId) {
          logger.info(
            `[SocketIO] Skipping socket ${clientSocket.id} - same as sender (${entityId})`
          );
          continue;
        }
  ...
  private async createMessageMemoryForAgent(
    ...
    logger.info(
      `[SocketIO] Creating memory for message from ${entityId} to agent ${runtime.agentId}`
    );
    ...
    try {
      ...
      try {
        ...
        logger.error(
          `[SocketIO] Error adding embedding: ${embeddingError.message}`,
          embeddingError
        );
      ...
      if (agentService) {
        logger.info(
          `[SocketIO] Found WebSocket service for agent ${runtime.agentId}, triggering message processing`
        );
        agentService.emit('processMessage', memory);
      } else {
        logger.info(
          `[SocketIO] Agent ${runtime.agentId} doesn't have a WebSocket service, relying on memory monitoring`
        );
      }
   ...
   */
  private getAgentWebSocketService(
    agentId: UUID
