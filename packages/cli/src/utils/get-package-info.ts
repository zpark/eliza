...
export async function getPackageVersion(packageName: string): Promise<string> {
  try {
    ...
    const monoRepoPackagePath = path.resolve(process.cwd(), '..', '..', 'packages', packageName.replace('@elizaos/', ''), 'package.json');
...
export async function getLocalPackages(): Promise<string[]> {
  ...
  try {
    ...
    const packagesDirEntries = await fs.readdir(path.join(packagesDir, 'packages'), { withFileTypes: true });
