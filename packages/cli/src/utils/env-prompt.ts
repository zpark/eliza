	...
	openai: [
		{
			...
			required: true,
			description:
				"Required for the OpenAI plugin to generate text and embeddings.",
	...
	twitter: [
		{
			...
			required: false,
			description:
				"API Key for Twitter integration. Needed to connect your agent to Twitter.",
	...
	telegram: [
		{
			...
			required: false,
			description:
				"Bot Token for Telegram integration. Needed to connect your agent to Telegram.",
...
export function checkEnvVarsForPlugin(pluginName: string): boolean {
	...
	for (const config of envVarConfigs) {
		if (
			config.required &&
			(!envVars[config.key] || envVars[config.key] === "dummy_key")
		) {
			return false;
		}
	}
...
export function validatePluginConfig(pluginName: string): {
	...
	switch (pluginName.toLowerCase()) {
		...
		case "twitter": {
			...
			const providedKeys = twitterKeys.filter(
		...
		case "telegram":
			if (
				...
				envVars.TELEGRAM_BOT_TOKEN.trim() !== ""
			) {
				return {
					valid: true,
		...
		case "openai":
			if (!envVars.OPENAI_API_KEY || envVars.OPENAI_API_KEY.trim() === "") {
				return {
					valid: false,
					message:
						"OpenAI API Key is not configured. This is required for using OpenAI models.",
				};
			}
			return {
				valid: true,
		...
		case "anthropic":
			if (
				...
				return {
					valid: false,
					message:
						"Anthropic API Key is not configured. This is required for using Claude models.",
				};
			}
			return {
				valid: true,
...
/**
 * Prompts the user to select which services and AI models they want to enable
 *
 * @returns An object containing the selected services and AI models
 */
export async function promptForServices(): Promise<{
	services: string[];
	aiModels: string[];
}> {
	...
	const serviceOptions = [
		{
			title: "Discord",
			value: "discord",
			description: "Connect to Discord channels and servers",
		},
		{
			title: "Twitter",
			value: "twitter",
		...
		},
		{
			title: "Telegram",
			value: "telegram",
			description: "Connect to Telegram chats",
		},
	];

	// Define AI model options
	const modelOptions = [
		{ title: "OpenAI (GPT models)", value: "openai", selected: true },
		{ title: "Anthropic (Claude models)", value: "anthropic" },
	];
	...
	const serviceResult = await prompts({
		type: "multiselect",
		name: "services",
		message: "Which messaging services would you like to enable?",
		choices: serviceOptions,
		hint: "↑/↓: Navigate, Space: Select/Deselect, Enter: Confirm",
	});
	...
	const modelResult = await prompts({
		type: "multiselect",
		name: "aiModels",
		message: "Which AI model providers would you like to use?",
