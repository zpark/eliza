...
async function updateDependencies(cwd: string, isPlugin: boolean): Promise<void> {
  ...
  try {
    ...
    const workspacePackages = elizaPackages.filter(pkg => isWorkspaceVersion(pkg.version as string));
    if (workspacePackages.length > 0) {
      ...
      const packagesToUpdate = elizaPackages.filter(pkg => !isWorkspaceVersion(pkg.version as string));
