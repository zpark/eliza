...
async function determineProjectType(): Promise<{ isProject: boolean; isPlugin: boolean }> {
  ...
  if (fs.existsSync(packageJsonPath)) {
    try {
      ...
      logger.info(`Package type check: ${JSON.stringify({
      ...
      if (
        ...
        (packageJson.name && (packageJson.name.includes("project-") || packageJson.name.includes("-org"))) ||
        (packageJson.keywords && Array.isArray(packageJson.keywords) && 
      ...
      if (!isProject && !isPlugin) {
        ...
        if (fs.existsSync(indexPath)) {
          ...
          if (indexContent.includes("export const project") || 
...
async function watchDirectory(dir: string, onChange: () => void): Promise<void> {
  ...
  try {
    ...
    if (tsFiles.length > 0) {
      logger.info(`Sample files: ${tsFiles.slice(0, 3).join(', ')}${tsFiles.length > 3 ? '...' : ''}`);
    }
    ...
    watcher.on("ready", () => {
      ...
      if (pathsCount === 0) {
        ...
        logger.info(`Top-level watched directories: ${Object.keys(watchedPaths).slice(0, 5).join(', ')}${Object.keys(watchedPaths).length > 5 ? '...' : ''}`);
      }
...
export const dev = new Command()
  .name("dev")
  .description("Start the project or plugin in development mode with auto-rebuild and restart on file changes")
