---
id: get-agent-servers
title: 'Get agent servers'
description: 'Retrieves all servers an agent belongs to'
sidebar_label: 'Get agent servers'
hide_title: true
...
<Heading as={'h1'} className={'openapi__heading'} children={'Get agent servers'}></Heading>

<MethodEndpoint
  method={'get'}
  path={'/api/messages/agents/{agentId}/servers'}
  context={'endpoint'}
></MethodEndpoint>

Retrieves all servers that an agent is associated with
...
<StatusCodes
  ...
  responses={{
    ...
    '500': {
      description: 'Error retrieving agent servers',
      content: {
        'application/json': {
          schema: {
            type: 'object',
