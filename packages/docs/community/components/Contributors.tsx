	...
	useEffect(() => {
		const fetchActivitySummaries = async () => {
			try {
				...
				const currentActivitySummaries = new Map<string, ActivityDetails>();
				specs.forEach(
					(spec: {
						contributor: string;
						score: number;
		...
		>
			{selectedContributor ? (
				<Contributions
					...
					darkMode={darkMode}
					activitySummary={
						activitySummaries.get(selectedContributor.login)?.activitySummary
					}
					score={activitySummaries.get(selectedContributor.login)?.score}
				/>
			) : (
				<>
					{contributors.map((contributor) => (
						<ContributorCard
							...
							darkMode={darkMode}
							activitySummary={
								activitySummaries.get(contributor.login)?.activitySummary
							}
							score={activitySummaries.get(contributor.login)?.score}
						/>
					))}
					<div
						ref={observerRef}
