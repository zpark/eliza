...
elizaos plugin installed-plugins

# Add a plugin to your project (with options)
elizaos plugin add <plugin-name> [--no-env-prompt] [--branch <branchName>]

# Remove a plugin from your project
elizaos plugin remove <plugin-name>
```

- `elizaos plugin list` (aliases: `ls`, `l`): Lists available plugins from the registry.
- `elizaos plugin installed-plugins`: Lists plugins found in your project's dependencies.
- `elizaos plugin add <plugin>` (alias: `install`): Adds a plugin to your project. Options:
...
- `elizaos plugin remove <plugin>` (aliases: `delete`, `del`, `rm`): Removes a plugin from your project and cleans up files.
