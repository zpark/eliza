...
V2 introduces significant upgrades to improve how developers build, share, and integrate functionality into their agents:


| V1 Approach | V2 Approach |
|-------------|-------------|
| Central monorepo | Independent repositories |
| PR approval workflow | Self-published plugins |
| Tightly coupled components | Clean, standardized interfaces |
| Manual dependency management | Automated plugin resolution |
| Limited extensibility | Comprehensive plugin ecosystem |

Here's a guide on how to add your own:
...
Access plugin settings and secrets using the runtime API:
...
Define settings in your agent configuration:
...
```json
{
  ...
  "settings": {
    "secrets": {
      "PLUGIN_API_KEY": "your-secret-key"
    }
  }
}
```
