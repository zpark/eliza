...
describe("Utils Comprehensive Tests", () => {
    describe("parseBooleanFromText", () => {
        it("should return true for affirmative values", () => {
            const affirmativeValues = ["YES", "Y", "TRUE", "T", "1", "ON", "ENABLE", 
                                      "yes", "y", "true", "t", " YES ", " true "];
        ...
        it("should return false for negative values", () => {
            const negativeValues = ["NO", "N", "FALSE", "F", "0", "OFF", "DISABLE",
                                   "no", "n", "false", "f", " NO ", " false "];
    ...
    describe("formatMessages", () => {
        ...
        it("should handle attachments", () => {
            ...
            expect(result).toContain("(Attachments: [att-1 - Image (http://example.com/img.jpg)], [att-2 - Document (http://example.com/doc.pdf)])");
        });
  ...
  */
  ...
  it('trimTokens truncates using runtime tokenizer', async () => {
    const runtime = {
      useModel: vi.fn(async (type: typeof ModelType[keyof typeof ModelType], { prompt, tokens }: any) => {
