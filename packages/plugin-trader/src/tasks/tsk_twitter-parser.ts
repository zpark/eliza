...
export default class TwitterParser {
  ...
  async fillTimeframe() {
    ...
    //console.log('intel:twtr-prsr::fillTimeframe - timeSlots', timeSlots)
    //console.log('intel:twtr-prsr::fillTimeframe - sentiments', sentiments)

    if (timeSlots.length > 0) {
      console.log('intel:twtr-prsr::fillTimeframe - filling time');
  ...
  async parseTweets() {
    ...
    if (!unprocessedSentiment) {
      logger.debug('intel:twtr-prsr::parseTweets - No unprocessed timeslots available.');
      return true;
    }

    logger.debug(
      `intel:twtr-prsr::parseTweets - Trying to process ${new Date(unprocessedSentiment.timeslot).toISOString()}`
    );
    ...
    if (!tweets || tweets.length === 0) {
      logger.info(
        `intel:twtr-prsr::parseTweets - No tweets to process for timeslot ${timeslot.toISOString()}`
      );
    ...
    console.log('intel:twtr-prsr::parseTweets - saving sentiments');
    await this.runtime.setCache<Sentiment[]>('sentiments', updatedSentiments);
