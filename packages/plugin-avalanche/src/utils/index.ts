...
import { createPublicClient, createWalletClient, Hash, http, Address, parseUnits } from "viem"
...
export const getDecimals = async (runtime: IAgentRuntime, tokenAddress: Address) => {
...
export const getNativeBalance = async (runtime: IAgentRuntime, owner: Address) => {
    const publicClient = getPublicClient(runtime)
...
export const getTokenBalance = async (runtime: IAgentRuntime, tokenAddress: Address, owner: Address) => {
    if (tokenAddress === "0x0000000000000000000000000000000000000000") {
...
export const getQuote = async (runtime: IAgentRuntime, fromTokenAddress: Address, toTokenAddress: Address, amount: number) => {
    const publicClient = getPublicClient(runtime)
...
export const sendNativeAsset = async (runtime: IAgentRuntime, recipient: Address, amount: number) => {
    const walletClient = getWalletClient(runtime)
    const decimals = await getDecimals(runtime, "0x0000000000000000000000000000000000000000")
...
export const sendToken = async (runtime: IAgentRuntime, tokenAddress: Address, recipient: Address, amount: number) => {
...
export const approve = async (runtime: IAgentRuntime, tokenAddress: Address, spender: Address, amount: number) => {
    try {
        const decimals = await getDecimals(runtime, tokenAddress);
        const publicClient = getPublicClient(runtime)
        const { result, request } = await publicClient.simulateContract({
...
export const swap = async (runtime: IAgentRuntime, quote: YakSwapQuote, recipient?: Address) => {
...
export const deposit = async (runtime: IAgentRuntime, depositTokenAddress: Address, strategyAddress: Address, amount: number) => {
    try {
