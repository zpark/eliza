import { getAccount, getWalletClient, getTxReceipt, getPublicClient } from "./index"
...
export const createMarketAndToken = async (runtime: IAgentRuntime, name: string, symbol: string) => {
    ...
    const params: TokenMillMarketCreationParameters = {
        ...
        creatorShare: 2000,
        stakingShare: 6000,
        bidPrices: [0,0.018117,0.042669,0.075735,0.12078,0.18018,0.26235,0.37124999999999997,0.51975,0.71973,0.99].map(price => parseUnits(price.toString(), 18)),
        askPrices: [0,0.0183,0.0431,0.0765,0.122,0.182,0.265,0.375,0.525,0.727,1].map(price => parseUnits(price.toString(), 18)),
        args: encodeAbiParameters([{ name: "decimals",type: "uint256" }], [18n])
    }
