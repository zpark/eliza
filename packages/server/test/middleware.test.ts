...
} from '../src/api/shared/middleware';
...
describe('Middleware Functions', () => {
  ...
  describe('agentExistsMiddleware', () => {
    ...
    it('should return 400 for invalid agent ID format', () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ error: { code: 'INVALID_ID', message: 'Invalid agent ID format' } });
    ...
    it('should return 404 when agent not found', () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ error: { code: 'NOT_FOUND', message: 'Agent not found' } });
      expect(next).not.toHaveBeenCalled();
    });
  });
  ...
  describe('validateUuidMiddleware', () => {
    ...
    it('should reject invalid UUID and log security event', () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith('[SECURITY] Invalid testId from 192.168.1.100: invalid-uuid');
      expect(res.status).toHaveBeenCalledWith(400);
      expect(next).not.toHaveBeenCalled();
    });
  ...
  describe('validateChannelIdMiddleware', () => {
    ...
    it('should return 400 when channelId is missing', () => {
      ...
      expect(res.status).toHaveBeenCalledWith(400);
      expect(res.json).toHaveBeenCalledWith({ error: { code: 'MISSING_CHANNEL_ID', message: 'Channel ID is required' } });
      expect(next).not.toHaveBeenCalled();
    });
    ...
    it('should reject invalid channel ID and log security event', () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith('[SECURITY] Failed channel ID validation from 192.168.1.100: invalid-channel-id');
      expect(res.status).toHaveBeenCalledWith(400);
      expect(next).not.toHaveBeenCalled();
    });
  });
  ...
  describe('securityMiddleware', () => {
    ...
    it('should detect suspicious User-Agent patterns', () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith('[SECURITY] Suspicious User-Agent from 192.168.1.100: Mozilla/5.0 <script>alert(1)</script>');
      expect(next).toHaveBeenCalled();
    });
    ...
    it('should detect path traversal attempts', () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith('[SECURITY] Path traversal detected from 192.168.1.100: /api/test/../../../etc/passwd');
      expect(next).toHaveBeenCalled();
    });
    ...
    it('should detect XSS attempts', () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining('[SECURITY] XSS attempt detected from 192.168.1.100'));
      expect(next).toHaveBeenCalled();
    });
    ...
    it('should detect SQL injection patterns', () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith('[SECURITY] SQL injection pattern detected from 192.168.1.100: /api/test?id=1 UNION SELECT * FROM users');
      expect(next).toHaveBeenCalled();
    });
  });
