...
describe("Discord MessageManager", () => {
  ...
  beforeEach(() => {
    ...
    const guild = {
      fetch: vi.fn().mockReturnValue({ type: ChannelType.GuildText, serverId: "mock-server-id" }),
      members: { cache: { get: vi.fn().mockReturnValue({ nickname: "MockBotNickname" }) } },
    };
  ...
  it("should process audio attachments", async () => {
    vi.spyOn(messageManager, "processMessage").mockResolvedValue({ processedContent: "", attachments: [] });
    
    const mockAttachments = new Collection<string, any>([[
      "mock-attachment-id",
      { attachment: "https://www.example.mp3", name: "mock-attachment.mp3", contentType: "audio/mpeg" },
    ]]);
