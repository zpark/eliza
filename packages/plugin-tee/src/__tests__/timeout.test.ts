...
describe('TEE Provider Timeout Tests', () => {
  ...
  describe('RemoteAttestationProvider', () => {
    it('should handle API timeout during attestation generation', async () => {
      const mockTdxQuote = vi.fn().mockRejectedValueOnce(new Error('Request timed out'));
      ...
      await expect(() => provider.generateAttestation('test-data')).rejects.toThrow(
        'Failed to generate TDX Quote: Request timed out'
      );
    ...
    it('should handle network errors during attestation generation', async () => {
      const mockTdxQuote = vi.fn().mockRejectedValueOnce(new Error('Network error'));
      ...
      const provider = new RemoteAttestationProvider(TEEMode.LOCAL);
      await expect(() => provider.generateAttestation('test-data')).rejects.toThrow(
        'Failed to generate TDX Quote: Network error'
      );
  ...
  describe('DeriveKeyProvider', () => {
    it('should handle API timeout during key derivation', async () => {
      const mockDeriveKey = vi.fn().mockRejectedValueOnce(new Error('Request timed out'));
      ...
      await expect(() => provider.rawDeriveKey('test-path', 'test-subject')).rejects.toThrow(
        'Request timed out'
      );
    ...
    it('should handle API timeout during Ed25519 key derivation', async () => {
      ...
      const provider = new DeriveKeyProvider(TEEMode.LOCAL);
      await expect(() =>
        provider.deriveEd25519Keypair('test-path', 'test-subject')
