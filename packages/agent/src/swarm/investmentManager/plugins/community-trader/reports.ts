...
function calculateTradeMetrics(
    ...
    for (const tx of transactions) {
        ...
        if (tx.timestamp < firstTradeTime)
            firstTradeTime = new Date(tx.timestamp);
        if (tx.timestamp > lastTradeTime)
...
function calculatePositionPerformance(
    ...
    const unrealizedPnLPercent =
        trades.averageEntryPrice > 0
            ? ((currentPrice - trades.averageEntryPrice) /
                  trades.averageEntryPrice) *
              100
            : 0;
        ...
        positions.reduce(
            (acc, position) => {
                ...
                if (token) {
                    ...
                    return {
                        totalCurrentValue:
                            acc.totalCurrentValue + perfData.currentValue,
                        totalRealizedPnL:
                            acc.totalRealizedPnL + perfData.trades.realizedPnL,
                        totalUnrealizedPnL:
                            acc.totalUnrealizedPnL + perfData.unrealizedPnL,
                    };
                }
