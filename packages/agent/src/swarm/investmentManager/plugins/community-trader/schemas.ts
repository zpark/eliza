...
export function transformTokenPerformance(dbToken: any, chain: string = "unknown"): TokenPerformance {
  const input = {
    ...
    initialMarketCap: typeof dbToken.initial_market_cap === 'number' ? dbToken.initial_market_cap : 0,
    currentMarketCap: typeof dbToken.current_market_cap === 'number' ? dbToken.current_market_cap : 0,
...
export function transformTransaction(dbTx: any, positionId: string = "unknown", chain: string = "unknown"): Transaction {
  ...
  const input = {
    ...
    price: typeof dbTx.price === 'string' ? Number(dbTx.price) : 
           typeof dbTx.price === 'number' ? dbTx.price : undefined,
    isSimulation: Boolean(dbTx.isSimulation || dbTx.is_simulation),
    timestamp: dbTx.timestamp instanceof Date ? dbTx.timestamp.toISOString() : 
               typeof dbTx.timestamp === 'string' ? dbTx.timestamp : 
...
export function transformPosition(dbPos: any): Position {
  const input = {
    ...
    initialMarketCap: dbPos.initialMarketCap?.toString() || dbPos.initial_market_cap?.toString() || "0",
    initialLiquidity: dbPos.initialLiquidity?.toString() || dbPos.initial_liquidity?.toString() || "0",
    performanceScore: typeof dbPos.performanceScore === 'number' ? dbPos.performanceScore : 
                      typeof dbPos.performance_score === 'number' ? dbPos.performance_score : 0,
    rapidDump: Boolean(dbPos.rapidDump || dbPos.rapid_dump),
    openedAt: dbPos.openedAt instanceof Date ? dbPos.openedAt : 
              dbPos.opened_at instanceof Date ? dbPos.opened_at : 
              typeof dbPos.openedAt === 'string' ? new Date(dbPos.openedAt) :
              typeof dbPos.opened_at === 'string' ? new Date(dbPos.opened_at) : new Date(),
    closedAt: dbPos.closedAt instanceof Date ? dbPos.closedAt : 
              dbPos.closed_at instanceof Date ? dbPos.closed_at : 
              typeof dbPos.closedAt === 'string' ? new Date(dbPos.closedAt) :
              typeof dbPos.closed_at === 'string' ? new Date(dbPos.closed_at) : undefined,
    updatedAt: dbPos.updatedAt instanceof Date ? dbPos.updatedAt : 
               dbPos.updated_at instanceof Date ? dbPos.updated_at : 
               typeof dbPos.updatedAt === 'string' ? new Date(dbPos.updatedAt) :
               typeof dbPos.updated_at === 'string' ? new Date(dbPos.updated_at) : new Date(),
