...
import { 
    CacheManager, 
    transformTokenPerformance, 
    transformRecommender,
    TokenPerformance,
    Recommender
} from "./schemas";

// Create a helper function to convert a string to a SQL wrapper
function column(name: string): SQLWrapper {
    return sql.identifier(name);
}

// Define schema with proper SQL wrapper functions
const schema = {
    positions: {
        id: column('id'),
        chain: column('chain'),
        tokenAddress: column('tokenAddress'),
        walletAddress: column('walletAddress'),
        isSimulation: column('isSimulation'),
        recommenderId: column('recommenderId'),
        recommendationId: column('recommendationId'),
        initialPrice: column('initialPrice'),
        initialMarketCap: column('initialMarketCap'),
        initialLiquidity: column('initialLiquidity'),
        performanceScore: column('performanceScore'),
        rapidDump: column('rapidDump'),
        openedAt: column('openedAt'),
        closedAt: column('closedAt'),
...
export class TrustScoreDatabase {
    ...
    constructor(runtime: IAgentRuntime) {
        ...
        this.cacheManager = new CacheManager(runtime);
  ...
  `);
    ...
    upsertTokenPerformance(performance: TokenPerformance): boolean {
        ...
        `;
        try {
            // Transform schema property names to database column names
            // Use default values for properties that don't exist in the new schema
            const liquidityChange24h = 0; // Default value since property doesn't exist in schema
            const marketCapChange24h = 0; // Default value since property doesn't exist in schema
            const balance = 0; // Default value since property doesn't exist in schema

            this.db.prepare(sql).run(
                performance.address,
                performance.price24hChange,
                performance.volume24hChange,
                performance.trades24hChange,
                performance.liquidity,
                liquidityChange24h,
                performance.holders24hChange,
                ...
                performance.validationTrust || 0,
                balance,
                performance.initialMarketCap,
                validationTrust
            );
            console.log(
                `Upserted token performance for ${performance.address}`
            );
            return true;
        } catch (error) {
     ...
     */
    async getTokenPerformance(chain: string, tokenAddress: string): Promise<TokenPerformance | null> {
        try {
            // First try to get from cache
            const cachedToken = await this.cacheManager.getTokenPerformance(chain, tokenAddress);
            if (cachedToken) {
                return cachedToken;
            }
            ...
            if (this.db.prepare) {
                ...
                const stmt = this.db.prepare(
                    "SELECT * FROM token_performance WHERE token_address = ? LIMIT 1"
                );
                tokenData = stmt.get(tokenAddress);
            } else {
                // Modern adapter
                try {
                    const query = `SELECT * FROM token_performance WHERE token_address = '${tokenAddress}' LIMIT 1`;
                    const rows = await this.db.query(query);
                    tokenData = rows?.[0];
                } catch (error) {
                    console.error("Error querying token performance:", error);
                    return null;
                }
            }
            ...
            // Transform to validated schema and cache
            const token = transformTokenPerformance(tokenData, chain);
            await this.cacheManager.cacheTokenPerformance(token);
  ...
  `;
    ...
    async getOpenPositionsWithBalance(): Promise<any> {
        const rows = await this.db
            .select()
            .from("positions")
            .where(sql`${schema.positions.closedAt} IS NULL`);
    ...
    async getPositionTransactions(positionId: UUID): Promise<Transaction[]> {
        const rows = await this.db
            .select()
            .from("transactions")
            .where(sql`${schema.transactions.positionId} = ${positionId}`);
    ...
    async getPositionsTransactions(
        ...
        const rows = await this.db
            .select()
            .from("transactions")
            .where(sql`${schema.transactions.positionId} IN (${sql.join(positionIds, sql`, `)})`);
    ...
    async updatePosition(position: Position): Promise<boolean> {
        try {
            await this.db
                ...
                .where(sql`${schema.positions.id} = ${position.id}`);
    ...
    async touchPosition(id: UUID) {
        try {
            await this.db
                ...
                .where(sql`${schema.positions.id} = ${id}`);
        } catch (_error) {}
    }
    ...
    async closePositions(ids: UUID[]) {
        try {
            await this.db
                ...
                .where(sql`${schema.positions.id} IN (${sql.join(ids, sql`, `)})`);
        } catch (_error) {}
    }
    ...
    async getMessagesByUserId(
        ...
        const rows = await this.db
            ...
            .from("memories")
            .where(
                sql`${schema.memories.userId} = ${userId} AND ${schema.memories.type} = ${"messages"}`
            )
            .orderBy(sql`${schema.memories.createdAt} DESC`)
            .limit(limit);
    ...
    /**
     * Get a recommender by platform and optional user ID
     */
    async getRecommenderByPlatform(
        ...
        try {
            // First try to get from cache
            const cachedRecommender = await this.cacheManager.getRecommender(platform, userId);
            if (cachedRecommender) {
                return cachedRecommender;
            }

            // If not in cache, fetch from database
            let recommenderData: any = null;

            if (this.db.prepare) {
                // SQLite adapter
                const stmt = this.db.prepare(
                    "SELECT * FROM recommenders WHERE platform = ? AND (user_id = ? OR ? IS NULL) LIMIT 1"
                );
                recommenderData = stmt.get(platform, userId, userId);
            } else {
                // Modern adapter
                try {
                    const query = `
                    ...
                    `;
                    const rows = await this.db.query(query);
                    recommenderData = rows?.[0];
                } catch (error) {
                    console.error("Error querying recommender:", error);
                    return null;
                }
            }

            if (!recommenderData) return null;

            // Transform to validated schema and cache
            const recommender = transformRecommender(recommenderData);
            await this.cacheManager.cacheRecommender(recommender);
            
            return recommender;
        } catch (error) {
            console.error("Error getting recommender by platform:", error);
            return null;
        }
    }
}