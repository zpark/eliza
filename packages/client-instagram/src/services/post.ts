...
export class InstagramPostService {
  ...
  async start() {
    const generatePostLoop = async () => {
      ...
      const minMinutes = parseInt(this.runtime.getSetting('POST_INTERVAL_MIN') || '90', 10);
      const maxMinutes = parseInt(this.runtime.getSetting('POST_INTERVAL_MAX') || '180', 10);
      ...
      elizaLogger.log(`Next Instagram post scheduled in ${randomMinutes} minutes`);
    };
