    ...
    implements IDatabaseCacheAdapter
{
    ...
    private async validateVectorSetup(): Promise<boolean> {
        try {
            ...
            logger.error("Failed to validate vector extension:", {
    ...
    async createAccount(account: Account): Promise<boolean> {
        return this.withDatabase(async () => {
            try {
                ...
                await this.pool.query(
                    `INSERT INTO accounts (id, name, username, email, "avatarUrl")
                    VALUES ($1, $2, $3, $4, $5)`,
                    [
                        accountId,
                        account.name,
    ...
    async createMemory(memory: Memory, tableName: string): Promise<void> {
        return this.withDatabase(async () => {
            logger.debug("PostgresAdapter createMemory:", {
                memoryId: memory.id,
    ...
    async getMemories(params: {
        ...
        return this.withDatabase(async () => {
            ...
            logger.debug("Fetching memories:", {
                roomId: params.roomId,
                tableName: params.tableName,
                unique: params.unique,
                agentId: params.agentId,
        ...
        return this.withDatabase(async () => {
            ...
            try {
                ...
                if (checkResult.rowCount === 0) {
                    logger.warn("No room found to remove:", { roomId });
                ...
                await client.query("ROLLBACK");
                logger.error("Failed to remove room:", {
                    roomId,
                    error:
                        error instanceof Error ? error.message : String(error),
                });
    ...
    async getRelationship(params: {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                if (rows.length > 0) {
                    logger.debug("Relationship found:", {
    ...
    async getCachedEmbeddings(opts: {
        ...
        return this.withDatabase(async () => {
            try {
                logger.debug("Fetching cached embeddings:", {
                ...
                logger.debug("Retrieved cached embeddings:", {
                    count: rows.length,
                    tableName: opts.query_table_name,
                    matchCount: opts.query_match_count,
                });
    ...
    async log(params: {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                logger.debug("Log entry created:", {
                    logId,
                    type: params.type,
                    roomId: params.roomId,
                    userId: params.userId,
    ...
    async searchMemoriesByEmbedding(
        ...
        return this.withDatabase(async () => {
            ...
            let sql = `
                SELECT *,
                1 - (embedding <-> $1::vector(${embedding.length})) as similarity
                FROM memories
                WHERE type = $2
            `;
